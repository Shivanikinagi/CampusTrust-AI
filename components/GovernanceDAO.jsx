...
export default function GovernanceDAO({ walletAddress, signCallback }) {
  ...
  const handleCreate = async () => {
    ...
    setStatus({ type: 'info', message: '🚀 Proposing new governance action on blockchain...' });

    try {
      // Create cryptographic hash of proposal data
      const proposalData = {
      ...
      // Record proposal on blockchain with gasless transaction
      ...
      const proposal = {
        ...
        signatures: [walletAddress.substring(0, 8) + '...'],
        status: 'active',
      ...
      setStatus({ type: 'success', message: `✅ Proposal Created! TX: ${result.txId}. Waiting for other signers.` });
    } catch (error) {
  ...
  const handleSign = async (proposalId) => {
    ...
    try {
      ...
      const voteHash = await gaslessService.createDataHash(JSON.stringify(voteData));

      // Record vote on blockchain with gasless transaction
      const result = await gaslessService.sendGaslessPayment(
        walletAddress,
        walletAddress,
        0.001,
      ...
      const updatedProposals = proposals.map(p => {
        if (p.id === proposalId) {
          const newSigs = [...p.signatures, walletAddress.substring(0, 8) + '...'];
          ...
          return {
            ...
            executionTxId: isExecuted ? result.txId : null,
            voteTxId: result.txId
          };
        }
        return p;
      });
      ...
      if (executedProposal.status === 'executed') {
        setStatus({ type: 'success', message: `✅ Quorum Reached (${daoState.requiredSignatures}/${daoState.totalSigners})! Transaction Executed. TX: ${executedProposal.executionTxId}` });
      } else {
        setStatus({ type: 'success', message: `✅ Signature Confirmed. TX: ${result.txId}. Waiting for ${daoState.requiredSignatures - executedProposal.signatures.length} more signer(s).` });
      }
    } catch (error) {
      setStatus({ type: 'error', message: error.message });
    }
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {proposals.map(proposal => {
          ...
          return (
            <div key={proposal.id} className={`rounded-2xl border p-6 transition-all ${isExecuted
                ? 'bg-green-900/10 border-green-500/30'
                : 'bg-gray-800/50 border-gray-700/50 hover:border-cyan-500/30'
              }`}>
              <div className="flex justify-between items-start mb-4">
                <span className={`px-2 py-1 rounded text-xs font-mono font-bold ${isExecuted ? 'bg-green-500/20 text-green-300' : 'bg-blue-500/20 text-blue-300'
                  }`}>
                  #{proposal.id}
                </span>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={isExecuted}
                  className={`flex-1 py-3 rounded-xl font-bold transition-all ${isExecuted
